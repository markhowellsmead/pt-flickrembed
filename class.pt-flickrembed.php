...
class PTFLICKREMBED{
	...
	var $key 				= 'pt-flickrembed',
		$debug 				= false,
		$imageSrcAttribute 	= 'data-original',
		$memory_limit		= 0,
		$maxImageSize 		= 2560,
		$maxThumbSize 		= 1024,
		$maxFullSize 		= 2560,
		$screenSize 		= -1,
		$flickrSizes 		= array('o', 'l', 'c', 'z', 'm', 'n', 's', 'q', 't', 'sq'), // largest size first per https://www.flickr.com/services/api/misc.urls.html
		$imageurl			= '', // only single view at the moment
		$upload_dir 		= '',
		$storagepath		= '/flickr/%1$s/%2$s_%3$s%4$s.jpg', // use with sprintf. this will be appended to upload_dir['baseurl']
		$flickrpath			= 'https://farm%1$s.static.flickr.com/%2$s/%3$s_%4$s%5$s.jpg', 	// use with sprintf.
		$image_tag			= '<img %1$s="%2$s" alt="%3$s" %4$s />', 						// use with sprintf.
		$image_wrapper_tag	= '<div class="wp-caption alignnone pt-flickrembed flickr size-%1$s">%2$s</div>',
		$flickrfolder		= '/flickr/%1s/', 												// use with sprintf.
		$atts 				= array(),
		$image_data 		= null,
		$images 			= null,
		$logfile			= '',
		$flickrUserName		= '',
		$cache_folder_path	= '',
		$cache_key			= '_UNDEFINED_CACHE_FILE_NAME.json',
		$config 			= array();
	...
	public function __construct(){
		...
		if( !empty($this->config['flickr_key']) && !empty($this->config['flickr_secret']) && !empty($this->config['flickr_userid']) ) {
			...
			$this->backupfolder = $this->upload_dir['basedir'].'/flickr_backup';
			$this->backupfolderURL = $this->upload_dir['baseurl'].'/flickr_backup';
...
*/
	...
	private function setCacheFolderPath(){
		...
		if(!is_dir($this->cache_folder_path)){
			if(!@mkdir($this->cache_folder_path,0755,true)){return '<p>The cache folder for ' .$this->key. ' could not be created.</p>';}
	...
	private function fetchFileFromFlickr($data, $size, $storagepath){
		...
		$flickrFolder = sprintf($this->flickrfolder,
			$data['photo']['server']
		);

		@mkdir($this->upload_dir['basedir'].$flickrFolder, 0755, true);

		$flickrURL = sprintf($this->flickrpath,
			$data['photo']['farm'],
			$data['photo']['server'],
			$data['photo']['id'],
			$data['photo']['secret'],
	...
	private function IDtoURL(&$data,$size,$single=false){
		...
		if($single){
			...
			$localimagepath = sprintf($this->storagepath,
				$data['photo']['server'],
				$data['photo']['id'],
				$data['photo']['secret'],
				($size!==''?'_'.$size:'')
		...
		if($this->atts['mode'] !== 'src'){
			...
			if ( false === ($html = get_transient($transient_name)) ){
				...
				if(!empty($html)){
					...
					$html = sprintf($this->image_wrapper_tag,
						$this->attr['size'],
						$html
					);
...
*/
		...
		if(!empty($out)){
			...
			return sprintf($this->image_wrapper_tag,
				$this->attr['size'],
				$html
			);
			...
			if(($image_data=$this->getRemoteFileContents($FlickrRequestString))){
				...
				if($images['stat']=='ok'){
					...
					foreach($photoset as $photo){
						...
						$out[] = sprintf($this->image_wrapper_tag,
							$this->attr['size'],
							$oembedhtml
						);
					}
	...
	private function backupLargestFlickrFile($photo_id){
		...
		if($imageinfo && isset($imageinfo['photo']) && isset($imageinfo['photo']['secret'])){
			...
			$sizesRequestString='https://api.flickr.com/services/rest/?method=flickr.photos.getSizes&format=json&nojsoncallback=1&api_key='.$this->config['flickr_key'].'&secret='.$imageinfo['photo']['secret'].'&photo_id='.$photo_id;
			...
			if( strpos($sizes['sizes']['size'][0]['url'], 'mhowells') && !file_exists( $this->backupfolder . parse_url($largestURL, PHP_URL_PATH ) )){
	...
	private function multisort(&$arr, $key, $sortDirection=SORT_ASC, $case_insensitive=true){
		/*
			via http://www.php.net/manual/en/function.usort.php#97750
			sort array $arr by array key $key in sort direction $sortDirection
			sorts referenced array so no return necessary
			29.11.2011
		*/

		if(is_array($arr) && (count($arr)>0) && ( ( is_array($arr[0]) && isset($arr[0][$key]) ) || ( is_object($arr[0]) && isset($arr[0]->$key) ) ) ){
